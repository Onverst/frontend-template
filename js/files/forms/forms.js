...
export let formValidate = {
	...
	validateInput(formRequiredItem) {
		...
		if (formRequiredItem.dataset.required === "email") {
			...
			if(formRequiredItem.value == '') {
				formRequiredItem.setAttribute('data-error', 'Пожалуйста, заполните это поле');
				this.addError(formRequiredItem);
				error++;
			} else {
				if (this.emailTest(formRequiredItem)) {
					formRequiredItem.setAttribute('data-error', 'Введен не верный E-mail');
					this.addError(formRequiredItem);
					error++;
				} else {
			...
			if(formRequiredItem.value == '') {
				...
				if (this.phoneTest(formRequiredItem)) {
					formRequiredItem.setAttribute('data-error', 'Введен не верный номер');
					this.addError(formRequiredItem);
					error++;
				} else {
					this.removeError(formRequiredItem);
	...
	phoneTest(formRequiredItem) {
		return formRequiredItem.value.replace(/[^0-9]/g,"").length < 11;
	}
}
/* Отправка форм */
